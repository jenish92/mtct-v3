...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    if($req_update)
    {
        ...
        $help_request_medical = array_diff_key($data_to_update,array_flip($help_request_keys));
        $help_request_medical['modified_by'] = $_SESSION['user_logged_in'];
        $db->where('requestor_id',$id);
        $stat = $db->update('mtct_medical_help', $help_request_medical);
        
        if($stat)
        {
        $_SESSION['success'] = "Medical help request updated successfully!";
...
if($edit)
{
    $db->join('mtct_medical_help md', 'md.requestor_id = hr.id', 'LEFT');
    $db->where('hr.id', $id);
    //Get data to pre-populate the form.
    $medical = $db->getOne("mtct_help_requested hr");
...
<div id="page-wrapper">
    <div class="row">
        <h2 class="page-header">Update Medical Help Request</h2>
    </div>
    <!-- Flash messages -->
    <?php
    ...
    <form class="" action="" method="post" enctype="multipart/form-data" id="donation_form">
        ...
        <?php
            ...
            require_once('./forms/medical_helps_form.php'); 
        ?>
    </form>
</div>
